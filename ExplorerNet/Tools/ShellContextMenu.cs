...
namespace ExplorerNet.Tools
{
    ...
    public class ShellContextMenu : NativeWindow
    {
        ...
        protected IntPtr[] GetPIDLs(FileSystemInfo[] arrFI)
        {
            ...
            if (arrFI[0].GetType() == typeof(DirectoryInfo))
            {
                oParentFolder = GetParentFolder((arrFI[0] as DirectoryInfo).Parent.FullName);
                
            }
            else
            {
                oParentFolder = GetParentFolder((arrFI[0] as FileInfo).DirectoryName);
            }
        ...
        protected IntPtr[] GetPIDLs(DriveInfo drive)
        {
            DirectoryInfo rootDir = drive.RootDirectory;


            IShellFolder oParentFolder = GetParentFolder("");
            if (null == oParentFolder)
            {
                return null;
            }

            IntPtr[] arrPIDLs = new IntPtr[1];
            int n = 0;
            //foreach (DirectoryInfo fi in arrFI)
            //{
                // Get the file relative to folder
                ...
                SFGAO pdwAttributes = 0;
                IntPtr pPIDL = IntPtr.Zero;
                int nResult = oParentFolder.ParseDisplayName(IntPtr.Zero, IntPtr.Zero, rootDir.Name, ref pchEaten, out pPIDL, ref pdwAttributes);
                if (S_OK != nResult)
                {
                    FreePIDLs(arrPIDLs);
                    return null;
        ...
        public void ShowContextMenu(FileSystemInfo[] fsies, Point pointScreen)
        {
            // Release all resources first.
            ReleaseAll();
            _arrPIDLs = GetPIDLs(fsies);
            this.ShowContextMenu(pointScreen);
        }

        public void ShowContextMenu(DriveInfo drive, Point pointScreen)
        {
            // Release all resources first.
            ReleaseAll();
            _arrPIDLs = GetPIDLs(drive);
            this.ShowContextMenu(pointScreen);
        }
